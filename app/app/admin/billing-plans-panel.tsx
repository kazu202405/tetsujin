  ...
  const save = async (code: string) => {
    ...
    try {
      const response = await fetch("/api/admin/billing-plans", {
        ...
        body: JSON.stringify({ code, stripePriceId: drafts[code] ?? "" }),
      });
          ...
          <div key={plan.code} className="rounded-xl border border-gray-100 p-3">
            ...
            <div className="flex gap-2">
              <input
                ...
                placeholder="price_..."
                className="flex-1 min-w-0 px-3 py-2 border border-gray-200 rounded-lg text-xs font-mono focus:outline-none focus:ring-2 focus:ring-gray-900"
              />
              <button
                onClick={() => save(plan.code)}
                disabled={savingCode === plan.code}
                className="px-4 py-2 rounded-lg bg-gray-900 text-white text-xs font-bold hover:bg-gray-800 disabled:opacity-40 flex-shrink-0"
              >
                保存
              </button>
            </div>
